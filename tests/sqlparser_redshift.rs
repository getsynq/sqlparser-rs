...
#[test]
fn parse_redshift_identity_seed_step() {
    // Redshift's two-argument IDENTITY shorthand: `(seed, step)` instead
    // of `(START WITH seed INCREMENT BY step)`.
    // https://docs.aws.amazon.com/redshift/latest/dg/r_CREATE_TABLE_NEW.html
    redshift()
        .parse_sql_statements("CREATE TABLE t (c BIGINT GENERATED BY DEFAULT AS IDENTITY (0, 1))")
        .unwrap();
    redshift()
        .parse_sql_statements("CREATE TABLE t (c BIGINT GENERATED ALWAYS AS IDENTITY (100, 5))")
        .unwrap();
    // Existing keyword form still parses.
    redshift()
        .parse_sql_statements(
            "CREATE TABLE t (c BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1))",
        )
        .unwrap();
}
