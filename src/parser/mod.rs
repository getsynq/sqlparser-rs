...
const DEFAULT_REMAINING_DEPTH: usize = 61;
...
impl<'a> Parser<'a> {
    ...
    pub fn parse_statement(&mut self) -> Result<Statement, ParserError> {
        ...
        match &next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                Keyword::DESCRIBE | Keyword::DESC => Ok(self.parse_explain(true)?),
    ...
    pub fn parse_wildcard_expr(&mut self) -> Result<WildcardExpr, ParserError> {
        ...
        let next_token = self.next_token();
        match next_token.token {
            Token::Word(w) if self.peek_token_is(&Token::Period) => {
    ...
    pub fn parse_prefix(&mut self) -> Result<Expr, ParserError> {
        ...
        let expr = match next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                Keyword::FLOOR if self.peek_token_is(&Token::LParen) => {
                    self.parse_ceil_floor_expr(false)
                }
                Keyword::POSITION if self.peek_token_is(&Token::LParen) => {
                    self.parse_position_expr(w.to_ident())
                }
                Keyword::SUBSTR | Keyword::SUBSTRING if self.peek_token_is(&Token::LParen) => {
                ...
                _ => match self.peek_token_kind().clone() {
            ...
            Token::Sharp if dialect_of!(self is DuckDbDialect) => {
                ...
                match self.peek_token().token {
                    ...
                    _ => self.expected("a number after #", self.peek_token())?,
                }
            }
    ...
    pub fn parse_window_frame_bound(&mut self) -> Result<WindowFrameBound, ParserError> {
        if self.parse_keywords(&[Keyword::CURRENT, Keyword::ROW]) {
            ...
            let rows = if self.parse_keyword(Keyword::UNBOUNDED) {
                ...
                Some(Box::new(match self.peek_token_kind().clone() {
    ...
    fn parse_group_by_with_modifiers(&mut self) -> Result<Vec<GroupByWithModifier>, ParserError> {
    ...
    /// ClickHouse also supports `CAST(expr, 'type')` syntax.
    pub fn parse_cast_expr(&mut self) -> Result<Expr, ParserError> {
        self.expect_token(&Token::LParen)?;
        let expr = self.parse_expr()?;

        // ClickHouse supports CAST(expr, 'type') with comma syntax
        if dialect_of!(self is ClickHouseDialect | GenericDialect)
            && self.consume_token(&Token::Comma)
        {
            let type_str = self.parse_literal_string()?;
            self.expect_token(&Token::RParen)?;
            // Parse the type string as a data type
    ...
    pub fn parse_trim_expr(&mut self) -> Result<Expr, ParserError> {
        ...
        if let Token::Word(word) = self.peek_token_kind().clone() {
    ...
    pub fn parse_interval(&mut self) -> Result<Expr, ParserError> {
        ...
        let leading_field = match self.peek_token_kind().clone() {
            Token::Word(kw)
                if [
                    Keyword::YEAR,
                    Keyword::YEARS,
    ...
    fn parse_bigquery_struct_literal(&mut self) -> Result<Expr, ParserError> {
        let (fields, trailing_bracket) =
            self.parse_struct_type_def(Self::parse_struct_field_def, Keyword::STRUCT)?;
        ...
        let values = if self.peek_token_kind() == &Token::RParen {
            ...
            self.parse_comma_separated(|parser| parser.parse_struct_field_expr(!fields.is_empty()))?
        };
        self.expect_token(&Token::RParen)?;
    ...
    fn parse_struct_field_def(
        ...
        let field_name: Option<WithSpan<Ident>> =
            if let Token::Word(_) = self.peek_token_kind().clone() {
    ...
    /// Static EOF token for returning references (avoids allocation)
    fn get_eof_token() -> &'static TokenWithLocation {
    ...
    /// Return reference to the first non-whitespace token (optimized, no clone)
    /// This is an internal optimization to avoid cloning tokens.
    #[inline]
    pub(crate) fn peek_token_ref(&self) -> &TokenWithLocation {
        self.peek_nth_token_ref(0)
    }

    /// Return reference to nth non-whitespace token (optimized, no clone)
    /// This is an internal optimization to avoid cloning tokens.
    #[inline]
    pub(crate) fn peek_nth_token_ref(&self, mut n: usize) -> &TokenWithLocation {
        let mut index = self.index;
        loop {
            index += 1;
            match self.tokens.get(index - 1) {
                ...
                Some(token) => {
                    if n == 0 {
                        return token; // No clone!
                    }
                    n -= 1;
                }
                None => return Self::get_eof_token(),
            }
        }
    }

    /// Return reference to token kind only (most common pattern, optimized)
    /// This is an internal optimization to avoid cloning tokens.
    #[inline]
    pub(crate) fn peek_token_kind(&self) -> &Token {
        &self.peek_token_ref().token
    }

    /// Check if peek_token matches a specific token (optimized, no clone)
    #[inline]
    pub(crate) fn peek_token_is(&self, expected: &Token) -> bool {
        self.peek_token_ref() == expected
    }
    ...
    fn is_parse_comma_separated_end(&mut self) -> bool {
        if !self.consume_token(&Token::Comma) {
            ...
            let token = self.peek_token_kind().clone();
            match token {
                Token::Word(ref kw)
    ...
    fn parse_create_function_inner(
        ...
        if dialect_of!(self is HiveDialect) {
            ...
            // or: CREATE FUNCTION name AS param -> body
            let name = self.parse_object_name(false)?;
            self.expect_keyword(Keyword::AS)?;
            let expr = self.parse_expr()?;
        ...
        loop {
            ...
            if self.parse_keyword(Keyword::AS) {
                ...
                body.using = Some(match keyword {
                    ...
                    Keyword::FILE => CreateFunctionUsing::File(uri),
    ...
    pub fn parse_create_view(
        ...
        if materialized {
            if self.parse_keywords(&[Keyword::PARTITION, Keyword::BY]) {
                // Skip the partition expression
                let _ = self.parse_expr();
            }
            // CLUSTER BY may appear after PARTITION BY or independently
            if self.parse_keywords(&[Keyword::CLUSTER, Keyword::BY]) {
                let _ = self.parse_comma_separated(|p| p.parse_expr());
            }
            // OPTIONS(...) may come before AS in BigQuery
        ...
        // Snowflake: CHANGE_TRACKING = TRUE|FALSE view property (before AS).
        // Consumed and discarded — metadata flag, no lineage relevance.
        loop {
            if self.parse_keyword(Keyword::CHANGE_TRACKING) {
                let _ = self.consume_token(&Token::Eq);
                let _ = self.next_token();
            } else if comment.is_none() && self.parse_keyword(Keyword::COMMENT) {
                let _ = self.consume_token(&Token::Eq);
                let next_token = self.next_token();
                comment = match next_token.token {
                    Token::SingleQuotedString(str) => Some(str),
    ...
    pub fn parse_declare(&mut self) -> Result<Statement, ParserError> {
        ...
        // Check if the next token looks like a data type (not cursor-related keywords)
        if dialect_of!(self is BigQueryDialect) {
            let idx = self.index;
            // Try parsing as variable declaration - check if next token is a type or comma
            if self.consume_token(&Token::Comma) || !self.parse_keyword(Keyword::CURSOR) {
                // Not a cursor declaration - skip to end of statement
                self.index = idx;
                // Skip comma-separated variable names
                while self.consume_token(&Token::Comma) {
    ...
    pub fn parse_columns(
        ...
        loop {
            ...
            } else if let Token::Word(_) = self.peek_token_kind().clone() {
    ...
    pub fn parse_optional_column_option(&mut self) -> Result<Option<ColumnOption>, ParserError> {
            ...
            && self.parse_keyword(Keyword::EPHEMERAL)
        {
            ...
            if matches!(self.peek_token_kind().clone(), Token::Comma | Token::RParen) {
            ...
            Token::Word(w) => match w.keyword {
                ...
                Keyword::DOUBLE => {
                    if self.parse_keyword(Keyword::PRECISION) {
                        ...
                        Ok(DataType::Double(self.parse_optional_precision()?))
                    }
                }
                Keyword::TINYINT => {
                ...
                Keyword::DATETIME => {
                    if dialect_of!(self is ClickHouseDialect) {
                        Ok(DataType::Datetime(None, self.parse_optional_timezone()?))
                    } else {
                        Ok(DataType::Datetime(self.parse_optional_precision()?, None))
                    }
                }
                Keyword::DATETIME64 => {
                    self.prev_token();
    ...
    pub fn parse_identifier_or_number(&mut self) -> Result<WithSpan<Ident>, ParserError> {
        let next_token = self.next_token();
        match next_token.token {
            Token::Number(ref n, _) => Ok(Ident::new(n.clone()).spanning(next_token.span)),
            _ => {
                self.prev_token();
    ...
    pub fn parse_parenthesized_column_list(
        ...
        if self.consume_token(&Token::LParen) {
            if allow_empty && self.peek_token_is(&Token::RParen) {
                self.next_token();
                Ok(vec![])
            } else {
                let cols = self
    ...
    pub fn parse_optional_timezone(&mut self) -> Result<Option<String>, ParserError> {
        if self.consume_token(&Token::LParen) {
            if self.consume_token(&Token::RParen) {
                return Ok(None);
            }
            let n = self.parse_literal_string()?;
            self.expect_token(&Token::RParen)?;
            Ok(Some(n))
        } else {
            Ok(None)
    ...
    pub fn parse_delete(&mut self) -> Result<Statement, ParserError> {
        let tables = if !self.parse_keyword(Keyword::FROM) {
            ...
            // BigQuery allows DELETE table_name [alias] WHERE ... without FROM
            if !self.parse_keyword(Keyword::FROM) {
                // No FROM keyword - treat the parsed table names as the FROM targets
    ...
    pub fn parse_explain(&mut self, describe_alias: bool) -> Result<Statement, ParserError> {
        ...
        let (explain_type, options) = if !describe_alias && dialect_of!(self is ClickHouseDialect) {
        ...
        match self.maybe_parse(|parser| parser.parse_statement()) {
            ...
            _ => {
                ...
                let function_params = if object_type_kw == Some(Keyword::FUNCTION)
                    && self.peek_token().token == Token::LParen
                {
                    self.expect_token(&Token::LParen)?;
    ...
    ///
    /// ClickHouse also supports `expr AS alias` style CTEs where the expression
    /// comes first, followed by `AS alias`.
    pub fn parse_cte(&mut self) -> Result<Cte, ParserError> {
        // ClickHouse supports `WITH expr AS alias` syntax where the expression
    ...
    /// Try to parse a ClickHouse-style CTE: `expr AS alias`
    /// Returns Err if this doesn't look like a ClickHouse CTE.
    fn try_parse_clickhouse_cte(&mut self) -> Result<Cte, ParserError> {
        let expr = self.parse_expr()?;
        self.expect_keyword(Keyword::AS)?;
        let alias_name = self.parse_identifier(false)?;

        // Wrap the expression in a SELECT query
        let query = Box::new(Query {
            with: None,
            body: Box::new(SetExpr::Select(Box::new(Select {
                distinct: None,
                top: None,
                projection: vec![SelectItem::UnnamedExpr(expr.spanning(Span::default()))
                    .spanning(Span::default())],
                into: None,
                from: vec![],
                lateral_views: vec![],
        ...
        let show_object_kind =
            if let Some(kw) = self.parse_one_of_keywords(&[Keyword::TABLE, Keyword::VIEW]) {
                match self.peek_token_ref().token {
                    Token::Word(_)
                    | Token::SingleQuotedString(_)
                    | Token::DoubleQuotedString(_) => Some(Ident::new(format!("{kw:?}"))),
                    _ => {
                        self.prev_token();
                        None
                    }
    ...
    pub fn parse_use(&mut self) -> Result<Statement, ParserError> {
        // Parse optional object type: DATABASE, SCHEMA, ROLE, WAREHOUSE, CATALOG, SECONDARY ROLES
        let object_type = match self.peek_token_kind().clone() {
            Token::Word(w) => {
                match w.value.to_uppercase().as_str() {
                    "DATABASE" | "SCHEMA" | "ROLE" | "WAREHOUSE" | "CATALOG" => {
                        self.next_token();
                    ...
                    "SECONDARY" => {
                        self.next_token();
                        // Expect ROLES
                        if let Token::Word(w2) = self.peek_token_kind().clone() {
                            if w2.value.to_uppercase() == "ROLES" {
                                self.next_token();
                                Some("SECONDARY ROLES".to_string())
                            } else {
                                // Not ROLES, put SECONDARY back as part of name
                                self.prev_token();
                                None
                            }
                        } else {
        ...
        loop {
            let join = if self.parse_keyword(Keyword::CROSS) {
                ...
                let join_operator_type = match peek_keyword {
                    ...
                    Keyword::ON => {
                        // Deferred ON clause for a preceding unconstrained join
                        // (nested-join syntax: FROM A JOIN B JOIN C ON cond1 ON cond2).
                        // Consume the ON and apply the constraint to the most recent
                        // unconstrained join; then continue looking for more joins.
                        ...
                        for join in joins.iter_mut().rev() {
                            ...
                            if needs_update {
                                let on = JoinConstraint::On(constraint);
        ...
        let grantee_type = match self.parse_one_of_keywords(&[
            ...
            Some(kw) => {
                ...
                match self.peek_token_ref().token {
                    Token::Word(_)
                    | Token::SingleQuotedString(_)
                    | Token::DoubleQuotedString(_) => match kw {
    ...
    pub fn parse_insert(&mut self) -> Result<Statement, ParserError> {
        ...
        if self.parse_keyword(Keyword::DIRECTORY) {
            ...
            let columns =
                if self.peek_token_is(&Token::LParen) && self.peek_parenthesized_query_start() {
                    vec![]
                    ...
                    && dialect_of!(self is ClickHouseDialect | GenericDialect)
                {
                    ...
                    if self.peek_token_is(&Token::Mul) {
                        self.next_token(); // consume *
                        self.expect_token(&Token::RParen)?;
        ...
        if self.consume_token(&Token::RParen) {
            ...
            let on_overflow = if self.parse_keywords(&[Keyword::ON, Keyword::OVERFLOW]) {
                if self.parse_keyword(Keyword::ERROR) {
                    ...
                    let filler = match self.peek_token_kind().clone() {
                        Token::Word(w)
    ...
    pub fn parse_create_procedure(&mut self, or_alter: bool) -> Result<Statement, ParserError> {
        ...
        if self.parse_keyword(Keyword::RETURNS) {
            // Consume the return type (could be TABLE(), VARCHAR, etc.)
            if self.consume_token(&Token::LParen) {
                // RETURNS TABLE (...) - skip balanced parens
                let _ = self.parse_data_type();
    ...
    pub fn parse_window_spec(&mut self) -> Result<WindowSpec, ParserError> {
        ...
        let window_name = if !self.peek_token_is(&Token::RParen)
            && !matches!(
                self.peek_token_kind().clone(),
